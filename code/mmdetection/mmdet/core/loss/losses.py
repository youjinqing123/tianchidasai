...
def weighted_cross_entropy(pred, label, weight, avg_factor=None, reduce=True):
    ...
    if reduce:
        return torch.sum(raw.float() * weight.float())[None] / avg_factor
    else:
        return raw.float() * weight.float() / avg_factor
...
def smooth_l1_loss(pred, target, beta=1.0, reduction='mean'):
    ...
    diff = torch.abs(pred.float() - target.float())
...
def weighted_smoothl1(pred, target, weight, beta=1.0, avg_factor=None):
    ...
    return torch.sum(loss.float() * weight.float())[None] / avg_factor
